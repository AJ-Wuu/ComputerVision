...
%STITCHIMG  Stitch multiple images into a single mosaic
    ...
    for n = 1:num_imgs
        ...
        % find the right homography to register img_n, and warping it with the reverse transformation (backward warp)
        [~, H] = runRANSAC(kp_stitched, kp_n, ransac_n, ransac_eps);
        [mask, dest_img] = backwardWarpImg(img_n, H, [size(stitched_img,2), size(stitched_img,1)]);
        
        % blend img_n into stitched_img
