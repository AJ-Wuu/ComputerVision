function out_img = blendImagePair(wrapped_imgs, masks, wrapped_imgd, maskd, mode)
    ...
    % convert to 'double' to avoid overflow/underflow when multiplying with the weighted mask
    ...
    for c = 1:Cs
        ...
        elseif strcmp(mode, 'blend')
            % compute the weighted masks (for src and dest)
            % bwdist(BW) computes the Euclidean distance transform of the binary image BW
            % note that 'euclidean' method is the default
            S_weighted_mask = bwdist(~binary_mask_s);
            D_weighted_mask = bwdist(~binary_mask_d);

            % form the output image
            % make sure that the mask weights sum to one in the valid region
            total_weighted_mask = S_weighted_mask + D_weighted_mask;
            total_weighted_mask(total_weighted_mask == 0) = 1; % need to update all 0 values to 1 for the sake of the next step
            channel_out = (S .* S_weighted_mask + D .* D_weighted_mask) ./ total_weighted_mask;
        end
